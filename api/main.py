...
async def job_new(req: Request, authorization: str = Header(None)):
    ...
    # ⚠️ **`auto` ကို ဖယ်လိုက်သည်** (၂၀၂၆-၀၉-၂၁ · Zin)。 အဲဒီလမ်းကြောင်းမှာ
    #    worker က review ကို ကျော်ပြီး တန်းထုတ်သဖြင့် ချောင်းဆိုး · ဖြည့်စကား ·
    #    ထပ်နေတာ · ပြန်စ အကြံပြုချက်တွေက **ထုတ်ပြီးမှ** report ထဲ ပေါ်သည် —
    #    「မထုတ်ခင် Transcript Review မှာ ပြရမည်」ဆိုသော စည်းကမ်းကို မပြည့်ပါ。
    #    UI က `mode` ကို တစ်ခါမှ မပို့သဖြင့် ဖယ်လိုက်တာ ဘာမှ မထိခိုက်ပါ。
    #    ⚠️ တောင်းဆိုမှုကို **တိတ်တဆိတ် မပြောင်းရ** — ငြင်းပြီး အကြောင်းရင်း ပြသည်。
    _md = (b.get("mode") or "review").strip()
    if _md and _md != "review":
