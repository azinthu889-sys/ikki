    ...
    if hues:
        ...
        for r, g, b, _c in hues[1:]:
            ...
            if min(abs(h2 - ah), 1 - abs(h2 - ah)) > 0.08:
                # ⚠️ ၂၀၂၆-၀၉-၁၉ ပြင်ချက် — ယခင်က `0.55/0.72` **ကိန်းသေ** ထားသဖြင့်
                #    တောက်ပသော ဘရန်း အရောင် (ဥပမာ ရွှေရောင် #F0B414 · s=0.92) က
                #    #B89C53 ဖြစ်ပြီး **မှိန်သွား**သည် (တိုင်းချက် — အကွာ ၈၈/၄၄၁)。
                #    accent နဲ့ **အတူတူ** — ဖတ်ရလွယ်ရန် **အောက်ခံ** သာ ထားသည်。
                s2, v2 = _hsv((r, g, b))[1], _hsv((r, g, b))[2]
                sec = _rgb(h2, max(s2, 0.55), max(v2, 0.62)); break
        if sec is None:
